	...
	$.fn.bstreetable = function(options){
        ...
        function generateTreeNode(curElement,row,curLevel,isPrepend){
            ...
            if(curLevel-0>=settings.maxlevel){
                $('<li class="j-expend"></li>').append('<label class="fa p-xs"></label>').append($("<input type='text' class='form-control input-sm' required/> <a href='javascript:void(0);' data-toggle=\"modal\" data-target=\"#categoryAdditionalFields\" class='manage_fields' > Manage Fields</a>").attr("data-oldval",row['name']).val(row['name']).attr("name","name")).attr('data-id',row.id).appendTo(dom_ul);
                dom_ul.attr("data-loaded",true);
            }
            else{
                $('<li class="j-expend"></li>').append('<label class="fa fa-plus p-xs"></label>').append($("<input type='text' class='form-control input-sm' required/> <a href='javascript:void(0);' data-toggle=\"modal\" data-target=\"#categoryAdditionalFields\" class='manage_fields'> Manage Fields</a>").attr("data-oldval",row['name']).val(row['name']).attr("name","name")).attr('data-id',row.id).appendTo(dom_ul);
            }
