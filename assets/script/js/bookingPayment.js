...
function processPaymentHandler() {
    var options = {
        ...
        "prefill": {
            "name": "Vikash Kumar",
            "email": "vikashranjan0000@gmail.com",
            "contact": "9738642264"
        },
        "notes": {
...
function callCreateBookingOrder(bookingData) {
    ...
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            var scheduleData = JSON.parse(response);
            if (scheduleData) {
                windows.localStorage.bookingData = JSON.stringify(bookingData);
                window.location.href = "/bookingPayment.php?orderId="+scheduleData['orderIdDetail'];    } else {
